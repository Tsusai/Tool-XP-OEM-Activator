        ...
        begin                // allow minus sign at start
            ...
            begin
                if NOT decimal then exit ;
								//if info [count] <> DecimalSeparator then exit ; //Del 7
								if info [count] <> FormatSettings.DecimalSeparator then exit ;
...
begin
    ...
    Temp := '';
    for I := 1 to L2 do
						//D7 Temp := ThousandSeparator + Copy (S, LS - 3 * I + 1, 3) + Temp;
						Temp := FormatSettings.ThousandSeparator + Copy (S, LS - 3 * I + 1, 3) + Temp;
...
begin
    ...
    while length (info) < 8 do info := '0' + info ;  // add leading zeros
		//D7 if info [6] <> TimeSeparator then exit ;
		if info [6] <> FormatSettings.TimeSeparator then exit ;
...
begin
    ...
    begin
		...
		//D7 result := IntToStr (hours) + TimeSeparator + info ;
		result := IntToStr (hours) + FormatSettings.TimeSeparator + info ;
end ;
